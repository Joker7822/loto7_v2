...
def save_self_predictions(predictions, file_path="self_predictions.csv", max_records=100):
    ...
    generation_file = f"self_predictions_gen_{timestamp}.csv"
...
class LotoPredictor:
    ...
    def train_model(self, data, accuracy_results=None):
        ...
        X, y, self.scaler = preprocess_data(past_data)
        ...
        try:
            ...
            # --- AutoML学習 ---
            print("[INFO] AutoML を使った機械学習モデルの学習開始")
            for i in range(7):
                try:
                    ...
                    self.regression_models[i] = None
                    print(f"[ERROR] AutoML モデル {i+1} の学習に失敗しました: {e}")

            # --- SetTransformer 学習 ---
            print("[INFO] SetTransformer の学習を開始")
            ...
            for epoch in range(300):
                ...
                optimizer.step()
                if (epoch + 1) % 100 == 0:
                    print(f"[SetTransformer] Epoch {epoch+1} Loss: {loss.item():.4f}")

            print("[INFO] SetTransformer モデルの学習完了")

            # --- スタッキング学習 ---
            print("[INFO] スタッキング融合モデルの学習を開始")
            true_labels = past_data['本数字'].tolist()
            lstm_preds = [nums for nums in true_labels]
            automl_preds = [nums for nums in true_labels]
            gan_preds = [convert_number_list_to_vector(nums) for nums in true_labels]
            ppo_preds = [convert_number_list_to_vector(nums) for nums in true_labels]
    ...
    def predict(self, latest_data, num_candidates=50):
        print("[INFO] 予測を開始（候補数:", num_candidates, "）")
        ...
        try:
            ...
            if not self.regression_models or any(m is None for m in self.regression_models):
                print("[ERROR] AutoML モデルが未学習または不完全です。train_model() を先に実行してください。")
                return None, None
            ...
            try:
                ...
                print(f"[ERROR] feature_metadata の取得に失敗: {e}")
                return None, None
            ...
            if actual_len > expected_len:
                X = X[:, :expected_len]
                print(f"[WARN] 特徴量切り詰め: {actual_len} → {expected_len}")
            elif actual_len < expected_len:
                X = np.hstack([X, np.zeros((X.shape[0], expected_len - actual_len))])
                print(f"[WARN] 特徴量パディング: {actual_len} → {expected_len}")
            ...
            for _ in range(num_candidates):
                try:
                    ml_predictions = np.array([
                        self.regression_models[i].predict(X_df) for i in range(7)
                    ]).T
                except Exception as e:
                    print(f"[ERROR] AutoML 予測エラー: {e}")
                    continue
                ...
                try:
                    ...
                    expected_input_size = self.lstm_model.lstm.input_size
                    if X.shape[1] < expected_input_size:
                        X_lstm = np.hstack([X, np.zeros((X.shape[0], expected_input_size - X.shape[1]))])
                    else:
                        X_lstm = X[:, :expected_input_size]

                    X_tensor = torch.tensor(X_lstm.reshape(-1, 1, expected_input_size), dtype=torch.float32).to(device)

                    with torch.no_grad():
                        lstm_predictions = self.lstm_model(X_tensor).detach().cpu().numpy()
                except Exception as e:
                    print(f"[ERROR] LSTM 予測エラー: {e}")
                    continue
            ...
            if self.gan_model is not None:
                try:
                    gan_samples = self.gan_model.generate_samples(num_candidates)
                    for sample in gan_samples:
                        numbers = np.argsort(sample)[-7:] + 1
                        numbers = np.sort(numbers)
                        score = sum(freq_score.get(n, 0) for n in numbers) - sum(cycle_score.get(n, 0) for n in numbers)
                        confidence = 0.8 + (score / 500.0)
                        all_predictions.append((numbers, confidence))
                except Exception as e:
                    print(f"[WARNING] GAN 予測エラー: {e}")
            ...
            if self.ppo_model is not None:
                try:
                    for _ in range(num_candidates):
                        ...
                        numbers = np.sort(numbers)
                        score = sum(freq_score.get(n, 0) for n in numbers) - sum(cycle_score.get(n, 0) for n in numbers)
                        confidence = 0.85 + (score / 500.0)
                        all_predictions.append((numbers, confidence))
                except Exception as e:
                    print(f"[WARNING] PPO 予測エラー: {e}")

            # SetTransformer予測
            if self.set_model is not None:
                try:
                    self.set_model.eval()
                    with torch.no_grad():
                        input_ids = torch.arange(1, 38).unsqueeze(0).to(device)
                        probs = self.set_model(input_ids).cpu().numpy()
                        for p in probs:
                            numbers = np.argsort(p)[-7:] + 1
                            numbers = np.sort(numbers)
                            score = sum(freq_score.get(n, 0) for n in numbers) - sum(cycle_score.get(n, 0) for n in numbers)
                            confidence = 0.9 + (score / 500.0)
                            all_predictions.append((numbers, confidence))
                except Exception as e:
                    print(f"[WARNING] SetTransformer 予測エラー: {e}")

            # Stacking予測（⚠️ダミー使用中）
            if self.stacking_model is not None:
                print("[INFO] スタッキング融合モデルによる予測を実行")
                try:
                    for _ in range(num_candidates):
                        # ⚠️ 過去の AutoML / LSTM / GAN / PPO の予測を使用すべき
                        lstm = np.random.choice(range(1, 38), 7, replace=False).tolist()
                        automl = np.random.choice(range(1, 38), 7, replace=False).tolist()
                        gan_vec = np.random.rand(37)
                        ppo_vec = np.random.rand(37)

                        numbers, _ = predict_with_stacking(
                            self.stacking_model, lstm, automl, gan_vec, ppo_vec
                        )

                        score = sum(freq_score.get(n, 0) for n in numbers) - sum(cycle_score.get(n, 0) for n in numbers)
                        confidence = 0.95 + (score / 500.0)
                        all_predictions.append((numbers, confidence))
                except Exception as e:
                    print(f"[WARNING] Stacking 予測エラー: {e}")
...
def evaluate_prediction_accuracy_with_bonus(predictions_file="loto7_predictions.csv", results_file="loto7_1.csv"):
    """
...
def main_with_improved_predictions():
    ...
    try:
        data = pd.read_csv("loto7_1.csv")
        print("データ読み込み完了")
    except Exception as e:
        print(f"データ読み込みエラー: {e}")
        return

    accuracy_results = evaluate_prediction_accuracy_with_bonus("loto7_predictions.csv", "loto7_1.csv")
...
def verify_predictions(predictions, historical_data, top_k=5):
    ...
    while len(selected) < (top_k - 2):  # ← ★あとで強制枠を確保するため-2
        best_score = -1
        best_idx = -1
    ...
    try:
        ...
        if high_match_rows:
            for _ in range(2):  # ← 2個強制追加
                template = random.choice(high_match_rows)
                template = set(template)
                available_numbers = list(set(range(1, 38)) - template)
                if available_numbers:
                    removed = random.choice(list(template))
                    added = random.choice(available_numbers)
                    template.remove(removed)
                    template.add(added)
                final_combo = sorted(template)
...
def bulk_predict_all_past_draws():
    ...
    for i in range(10, len(df)):
        ...
        print(f"\n=== {test_date_str} の予測を開始（{i}/{len(df)-1}） ===")

        # ✅ モデルを過去データのみで再学習
        try:
            X_tmp, _, _, _ = preprocess_data(train_data)
        except Exception as e:
            print(f"[ERROR] {test_date_str} の前処理中に例外発生: {e}")
            continue

        if X_tmp is None:
            print(f"[WARNING] {test_date_str} の学習データが不正なためスキップ")
            continue
        ...
        try:
            ...
            print(f"[ERROR] {test_date_str} モデル学習エラー: {e}")
            continue
        ...
        evaluate_prediction_accuracy_with_bonus("loto7_predictions.csv", "loto7_1.csv")
