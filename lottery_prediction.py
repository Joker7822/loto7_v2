# ==== [PATCH] Safe casting helpers to avoid 'only length-1 arrays...' ====
def _as_int_scalar(v):
    """配列やスカラーを安全に int に変換。空なら 0、配列は先頭を採用。"""
    import numpy as _np
    if _np.isscalar(v):
        try:
            return int(v)
...
def _to_int_list(x):
    ...
    for _v in a.tolist():
        try:
            ...
            try:
                out.append(int(_np.asarray(_v).ravel()[0]))
            except Exception:
                pass
    return out
# ==== [/PATCH] ====
...
# ==== Safe sampling helper (inserted automatically) ====
import numpy as _np

def _safe_choice_no_replace(population, size, p):
    pop = _np.arange(population) if isinstance(population, int) else _np.array(population)
    p = _np.array(p, dtype=float).reshape(-1)
    N = len(pop)
    if p.shape[0] != N:
        p = _np.ones(N, dtype=float) / N
    p[~_np.isfinite(p)] = 0.0
    p[p < 0] = 0.0
    s = p.sum()
    if s <= 0:
        p = _np.ones(N, dtype=float) / N
        ...
        p = p / s
    if (_np.where(p > 0)[0].size) < size:
        uniform = _np.ones(N, dtype=float) / N
        p = 0.5 * p + 0.5 * uniform
        p = p / p.sum()
    try:
        return _np.random.choice(pop, size=size, replace=False, p=p)
    except ValueError:
        draws = _np.random.choice(pop, size=size*5, replace=True, p=p)
        uniq = []
        seen = set()
        for v in draws:
            if v not in seen:
                ...
                if len(uniq) == size:
                    break
        if len(uniq) < size:
            remaining = [int(x) for x in pop if int(x) not in seen]
            extra = _np.random.choice(remaining, size=size-len(uniq), replace=False)
            uniq.extend([int(x) for x in extra])
        return _np.array(uniq[:size])
# ==== End helper ====
...
class LotoPredictor:
    ...
    def predict(self, latest_data, num_candidates=50):
        ...
        all_predictions = []  # [(numbers(list[int]), confidence(float)), ...]

        def append_prediction(numbers, base_confidence=0.8):
            # どんな形状でも 1 次元 7 個の [1..37] 整数に整える
            arr = np.asarray(numbers)
            arr = np.ravel(arr)
            arr = arr[np.isfinite(arr)]
            if arr.size == 0:
                return
            arr = np.clip(arr.astype(int), 1, 37)
            ...
            # 足りなければランダム補完（既出を避ける）
            if len(uniq) < 7:
                pool = [n for n in range(1, 38) if n not in seen]
                need = 7 - len(uniq)
                if need > 0 and pool:
                    uniq.extend(np.random.choice(pool, size=min(need, len(pool)), replace=False).tolist())
        
            nums = np.sort(uniq[:7]).tolist()
        
            # スコア・信頼度
            score = sum(freq_score.get(n, 0) for n in nums) - sum(cycle_score.get(n, 0) for n in nums)
            confidence = float(base_confidence + (score / 500.0))
            all_predictions.append((nums, confidence))
        try:
            # --- 特徴量整形 ---
            X_df = pd.DataFrame(X)
            ...
            # --- 伝統的回帰 + LSTM (+SetTransformer, TabNet) ---
            for i in range(num_candidates):
                ...
                # SetTransformer (任意)
                if getattr(self, "set_transformer_model", None):
            ...
            # --- GAN (任意) ---
            if getattr(self, "gan_model", None) is not None:
                for i in range(num_candidates):
                    set_global_seed(int(time.time() * 1000) % 100000 + i)
                    gan_sample = self.gan_model.generate_samples(1)[0]
                    # 温度スケーリング → 確率化 → 非復元サンプリング
                    logits = gan_sample / 0.7
                    probs = logits / (np.sum(logits) + 1e-12)
                    numbers = _safe_choice_no_replace(37, 7, probs) + 1  # 1..37 に合わせる
                    append_prediction(numbers, base_confidence=0.8)
            ...
            if getattr(self, "ppo_model", None) is not None:
                for i in range(num_candidates):
                    ...
                    append_prediction(numbers, base_confidence=0.85)

            # --- Diffusion (任意) ---
            if getattr(self, "diffusion_model", None) is not None:
            ...
            # --- BNN (任意) ---
            if getattr(self, "bnn_model", None) is not None:
                from bnn_module import predict_bayesian_regression
                print("[INFO] BNNモデルによる予測を実行中")
                for i in range(num_candidates):
                    set_global_seed(random.randint(1000, 999999))
                    try:
                        bnn_preds = predict_bayesian_regression(self.bnn_model, self.bnn_guide, X, samples=1)
                        for pred in bnn_preds:
                            ...
                            append_prediction(numbers[:7], base_confidence=0.83)
                    except Exception as e:
        ...
        # 重複排除（同一セットは最初の信頼度を採用）
        conf_map = {}
        for nums, c in all_predictions:
            t = tuple(nums)
            if t not in conf_map:
                conf_map[t] = float(c)

        numbers_only = [list(t) for t in conf_map.keys()]
        confidence_scores = [conf_map[tuple(n)] for n in numbers_only]
        ...
        try:
            ...
            # selection で並び替わる可能性があるので confidence を取り直す
            confidence_scores = [conf_map.get(tuple(n), 0.0) for n in numbers_only]

            # --- enforce_final_diversity が未定義なら簡易版を定義（フォールバック） ---
            if "enforce_final_diversity" not in globals():
                def enforce_final_diversity(candidates, max_sim=4/7):
                    # 何もしない素通し（最低限のフォールバック）
                    return candidates
            numbers_only = enforce_final_diversity(numbers_only, max_sim=4/7)
            confidence_scores = [conf_map.get(tuple(n), 0.0) for n in numbers_only]
        except Exception as e:
            print(f"[WARNING] 最終選抜中にエラー: {e}")
